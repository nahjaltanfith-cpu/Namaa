      ...
      <div className="min-h-screen">
        ...
        <section className="py-20 bg-background">
          <div className="container mx-auto px-4 max-w-5xl">
            <AnimatedSection>
              <div className="relative rounded-3xl overflow-hidden gradient-hero p-[1px]">
                <div className="rounded-3xl bg-background/95 backdrop-blur-xl p-8 md:p-12">
                  <div className="flex flex-col md:flex-row items-start gap-8">
                    <div className="w-16 h-16 rounded-2xl gradient-gold flex items-center justify-center shrink-0">
                      <Award className="text-gold-foreground" size={30} />
                    </div>
                    <div className="flex-1 space-y-6">
                      <h2 className="text-2xl md:text-3xl font-bold text-foreground">
                        {t.associationInfo.title[lang]}
                      </h2>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {[
                          { label: lang === "ar" ? "اسم الجمعية" : "Name", value: t.associationInfo.name[lang], icon: FileText },
                          { label: lang === "ar" ? "المقر" : "Headquarters", value: t.associationInfo.location[lang], icon: MapPin },
                          { label: lang === "ar" ? "نطاق الخدمات" : "Scope", value: t.associationInfo.scope[lang], icon: Compass },
                        ].map((item, i) => (
                          <motion.div
                            key={i}
                            whileHover={{ y: -2 }}
                            className="p-5 rounded-2xl bg-soft border border-border"
                          >
                            <item.icon className="text-gold mb-3" size={20} />
                      ...
                      <div className="p-4 rounded-xl bg-soft/60 border border-border/50">
                        <p className="text-xs text-muted-foreground leading-relaxed">{t.associationInfo.certificate[lang]}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="container mx-auto px-4 max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {t.about.strategicDirections.items.map((item, i) => {
                ...
                return (
                  <AnimatedSection key={i} delay={i * 0.1}>
                    <motion.div whileHover={{ y: -4, x: lang === "ar" ? -4 : 4 }} className="group flex items-center gap-5 p-6 rounded-2xl bg-background border border-border shadow-card hover:shadow-card-hover transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {t.about.strategicGoals.items.map((item, i) => (
                <AnimatedSection key={i} delay={i * 0.08}>
                  <motion.div whileHover={{ scale: 1.02 }} className="flex items-center gap-4 p-5 rounded-2xl bg-soft border border-border shadow-card">
          ...
          <div className="container mx-auto px-4 max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {t.about.workModel.beneficiaries.map((item, i) => {
                ...
                return (
                  <AnimatedSection key={i} delay={i * 0.1}>
                    <motion.div whileHover={{ y: -4 }} className="group p-8 rounded-2xl bg-background border border-border shadow-card hover:shadow-card-hover transition-all">
